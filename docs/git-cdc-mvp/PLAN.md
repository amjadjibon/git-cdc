...
- [ ] **1.1** Workspace member `crates/core` (package `git-cdc-core`) with deps: `fastcdc`,
  `blake3`, `anyhow`, `thiserror`. Types: `Chunk { hash, offset, length }`.
  *Done when*: `cargo build -p git-cdc-core` passes.
- [ ] **1.2** `chunker::chunk_stream(reader) -> anyhow::Result<(Vec<Chunk>, FileOid, u64)>`
...
- [ ] **1.3** `manifest` module: encode/parse the §15.1 format —
...
- [ ] **1.4** Tests: round-trip property test (random content →
...
- [ ] **2.1** `store::ChunkStore` trait — `has`, `put`, `get` only.
...
- [ ] **2.2** `DiskStore`: sharded layout `<root>/<hex[0..2]>/<hex[2..4]>/<hex>`,
...
- [ ] **2.3** Tests for 2.1–2.2 including the corruption-rejection case.
...
- [ ] **3.1** Workspace member `crates/server` with `axum`, `tokio`,
...
- [ ] **3.2** `POST /objects/batch`: LFS-shaped request with `operation`,
...
- [ ] **3.3** `PUT /chunks/{oid}` and `GET /chunks/{oid}` — basic transfer
...
- [ ] **3.4** Static bearer-token auth middleware on all routes.
  *Done when*: requests without/with-wrong token get 401.
- [ ] **3.5** Integration test: spin server on an ephemeral port, drive
...
- [ ] **4.1** `src/bin/git-cdc.rs` with `clap`: subcommands `install`,
...
- [ ] **4.2** `install [--global]`: set `filter.cdc.clean` / `filter.cdc.smudge`
...
- [ ] **4.3** `clean`: stdin → chunk (1.2) → write chunks to the local store
...
- [ ] **4.4** End-to-end filter test (shell or `assert_cmd`): scratch git
...
- [ ] **5.1** Batch client in `git-cdc-core` lib (`reqwest`, one shared `Client`):
...
- [ ] **5.2** `push`: enumerate every manifest blob across history via
...
- [ ] **5.3** `gc [--dry-run]` — client-driven mark-and-sweep per review
...
- [ ] **5.4** Full e2e test: scratch repo + live server; commit v1 of a
