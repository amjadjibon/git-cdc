...
- [ ] **1.1** `store::envelope`: `encode(raw) -> Vec<u8>` (zstd-3 with
...
- [ ] **1.2** `DiskStore` and `S3Store` store envelopes (`put` encodes,
...
- [ ] **2.1** HTTP: client uploads/downloads envelopes; server `put_chunk`
...
- [ ] **3.1** Hidden `git-cdc stdio --root <path>` serving the RESEARCH
  protocol over stdin/stdout (DiskStore + envelope underneath).
- [ ] **3.2** `Remote::Ssh` in the CLI: spawn `ssh <remote> git-cdc stdio
...
- [ ] **4.1** e2e_ssh suite; compression assertion (store bytes for a
  compressible asset < raw size; incompressible stays ~raw); legacy-store
  upgrade test.
- [ ] **4.2** Docs: `docs/spec/chunk-storage.md` (envelope format,
